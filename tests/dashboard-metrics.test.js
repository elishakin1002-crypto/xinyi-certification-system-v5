...
test('比率指标分母为 0 时显示「暂无数据」，不显示 0.0%', () => {
  ...
  */
  /*
    **盯的是"有几张比率卡、它们是不是都走 rate()"，不是卡片叫什么名字。**

    第一版把标题写成字面量（'本周日志覆盖率'），2026-09-18 按术语表
    把那张卡改名成「近七天日志覆盖率」（它的窗口本来就是回看七天，
    不是自然周），测试当场变红 —— 而行为一点没变。
    这就是 CLAUDE.md 说的「钉写法不钉意图」：改名要被 review 看到，
    但不该让一条讲"分母为 0 怎么显示"的测试红。

    改成：把所有 `value: xxx(分子, 分母)` 形式的卡片都找出来，
    要求它们一律走 rate()。新增一张比率卡忘了用 rate()，照样红。
  */
  const 比率卡 = [...SRC.matchAll(/title: ([`'][^`']*(?:率|覆盖率)[^`']*[`'][^,]*),\s*value: (\w+)\(/g)];
  assert.ok(比率卡.length >= 3, `只找到 ${比率卡.length} 张比率卡，少于预期 —— 是不是匹配写法过时了？`);
  for (const [, title, fn] of 比率卡) {
    assert.equal(fn, 'rate', `${title} 用的是 ${fn}()，分母为 0 时会显示 0.0%`);
  }
});
