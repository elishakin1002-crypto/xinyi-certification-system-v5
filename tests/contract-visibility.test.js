...
test('撞上重复时要说清是谁录的，并把筛选调到看得见它', () => {
  ...
  */
  /*
    2026-09-13 改文案后同步更新断言。

    原来第一句是「这份合同已经录过了，录入人：曾云俊」——
    金恩来读成了「我这次录入被记成曾云俊了」，去查为什么用黄佳佳的账号
    录入却算到别人头上（归属其实是对的）。
    问题出在句子的**主语**：那句讲的是"已存在那份"，
    但人正处在"我刚点了保存"的语境里，默认它在说自己这次操作。
    所以第一句必须是「你这次没有录入成功」。
  */
  const ctx = code('context/AppContext.tsx');
  assert.match(ctx, /你这次没有录入成功/, '第一句没有说清「这次没成功」，会被读成对本次操作的描述');
  assert.match(ctx, /已存在的那份/, '没有把主语切换到"已存在那份"');
  assert.match(ctx, /是 \$\{who\} 录的/, '重复提示没说是谁录的');
...
test('关联客户与服务项目并排 —— 右边不许空着，也不许把左边拉长', () => {
  /*
  ...
  */
  /*
    2026-09-12 三改。金恩来：「为什么把关联客户拉长，这样变得不协调和难看，
    为什么不把服务项目移动到原来客户名称的位置，这样不用移动到下面，
    也能看全下拉菜单了不是吗？」

    他是对的，而且一石二鸟：我上一版是**把洞撑开**（关联客户拉满整行），
    他说的是**把洞填上**，填进去的那个正好是
    「位置越高、下拉越有地方展开」的那个字段。
    改完下拉是向下开的，不再需要翻转。

    另外记一个坑：AI 对照那段原来是 grid 的第三个子元素，
    没内容时渲染 null，**但格子还在** —— 于是排成 [客户][空]、[服务][空]，
    服务项目被挤到第二行。**不可见 ≠ 不占位**，grid 里最容易忘这条。
  */
  const page = code('pages/Contracts.tsx');
  assert.ok(!/grid grid-cols-1 gap-4/.test(page),
    '又把某一行改成单列了 —— 那是「把洞撑开」，不是填上');
  // 关联客户和服务项目必须落在同一个 grid 容器里，且中间没有别的格子
  const at = page.indexOf('关联客户 <span');
  const seg = page.slice(at, page.indexOf('服务项目 <span', at));
  const cells = (seg.match(/^\s{26}<div>$/gm) || []).length;
  assert.equal(cells, 1, `关联客户和服务项目之间多出了 ${cells - 1} 个格子 —— 服务项目会被挤到下一行`);
});
