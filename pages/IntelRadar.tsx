...
const IntelRadar = () => {
  ...
  /*
    ── 「归档日报」的实现已删除（2026-09-14）────────────────────────

    按钮本身 2026-09-13 就下线了（金恩来：「情报雷达的内容有必要沉淀到
    知识中心吗？感觉有点多余」），但函数留着没删，成了**死代码地雷**。

    地雷在哪：它生成文档时用的是
        id: `DOC-INTEL-DIGEST-${Date.now()}`
    —— **每次调用都是新 id**。同一天点两次就在知识中心里躺两条一模一样的日报。
    2026-09-14 Codex 走查抓到的「同一篇日报显示两条」就是这么来的。

    删掉是因为：留着一个没人调、又自带去重 bug 的函数，
    下次谁想把这个功能接回去，第一件事就是复用它，bug 跟着回来。

    真要恢复这个功能，记住这条：**系统自动生成的文档必须用确定性 id**
    （比如 `DOC-INTEL-DIGEST-${日期}`），重复生成才会覆盖而不是新增。
    提醒那边的 dedupe_key 是同一个道理。
  */
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {/*
              ...
              实现也一并删了（2026-09-14）—— 它用 Date.now() 当文档 id，
              同一天点两次会在知识中心留两条一样的日报。真要接回来见上面那段说明。
            */}
            <button
