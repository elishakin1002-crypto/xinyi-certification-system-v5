// 批次3 数据迁移：state_store 的 contracts/settlements → PG。幂等：id 存在则跳过。
// 用法： XINYI_DB_URL=... node scripts/batch3-migrate.mjs [--dry]
import { createRequire } from 'module';
import fs from 'fs';
import path from 'path';
const require = createRequire(import.meta.url);
const { contractRepo } = require('../server/repos/contractRepo.js');
const { settlementRepo } = require('../server/repos/settlementRepo.js');
...
const run = async () => {
  ...
  let failed = 0;
  failed += await migrate('合同 contracts', loadDataset('contracts_v8'), contractRepo);
  failed += await migrate('结算 settlements', loadDataset('settlements_v8'), settlementRepo);
  console.log(failed ? `\n⚠️ 完成，但有 ${failed} 条失败` : '\n✅ 迁移完成，无失败');
