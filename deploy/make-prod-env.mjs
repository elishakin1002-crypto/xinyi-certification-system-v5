...
const PROD = {
  ...
  /*
    ── 会话闲置多久要重新登录（2026-09-05 由 7 天改成 12 小时）──

    原来是 604800000 = **7 天**，而且 SLIDING=true 每次使用还往后再续 7 天。
    实际效果是：一台电脑只要登过一次，之后直接敲地址就进去了 ——
    金恩来反馈的「另一台电脑不用登录就能进」就是这个，
    不是鉴权漏了，是会话几乎永不过期。

    办公室电脑是共用的。同事登过之后离开工位，
    下一个坐下来的人打开浏览器就能看到全公司的客户和合同金额。

    12 小时是「闲置」时长，不是绝对时长：白天一直在用就一直不掉线，
    晚上放着不动，第二天上班需要重新登一次。
    银行和主流 CRM 都是这个量级（8-12 小时），不是我自己拍的。
  */
  XINYI_SESSION_TTL_MS: '43200000',
  ...
  */
  ...
  /*
    管理通道：错误摘要、同事反馈、异常登录 —— **只发给系统管理员**。

    必须和业务通道分开，而且是两个不同的群：
      业务通道 → 工作群，收「浙江xx厂证书下月到期，该联系续证了」
      管理通道 → 管理员群（只有金恩来，或加上曾云俊）

    2026-09-03 差点搞错：配的第一个 webhook 指向企业全员群，
    13 个同事进来后会天天收到「Cannot read properties of undefined」——
    他们既看不懂也帮不上忙，几次之后就会开始无视这个群，
    连真正要紧的到期提醒也一起被无视。

    没配的话系统类通知**不发**，不会退回业务群。宁可漏发。
  */
  ADMIN_WEBHOOK_URL: '',
};
