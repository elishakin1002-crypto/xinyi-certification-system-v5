    ...
    if (existing) {
      ...
      */
      ...
      return {
        ok: false,
        /*
          第一句必须说「你这次没成功」（2026-09-13 改）。

          原来第一句是「这份合同已经录过了，录入人：曾云俊」——
          金恩来读成了「我这次录入被记成曾云俊了」，于是去查为什么
          用黄佳佳的账号录入却算到别人头上。
          （实际归属是对的：库里他建的两份，录入人就是黄佳佳。）

          问题出在句子的**主语**：那句话讲的是「已存在那份」的事，
          但人正处在"我刚点了保存"的语境里，会默认它在说自己这次操作。
          所以把「你这次没有录入成功」提到最前面，再说别的。
        */
        reason: `你这次没有录入成功 —— 这份合同之前已经录过了。\n\n`
          + `已存在的那份${who ? `是 ${who} 录的` : ''}${when ? `，签订日期 ${when}` : ''}，`
          + `已经帮你打开它（范围自动切到「全公司」）。\n\n`
          + `如果你手上这份其实是另一份合同（比如续签），改一下合同编号再录。`,
