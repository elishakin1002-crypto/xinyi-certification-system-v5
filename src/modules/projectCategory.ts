...
export const SCOPE_FILTERS = [
  ...
  */
  /*
    ...
    他看到的是工作台写 2、点进来变成 3 —— 差的就是那个待指派项目。
    工作台问的是「我今天有几个活」，那**不该**把没人认领的算进去；
    而这一页的「与我相关」故意要带上它（否则它会一直烂在那儿）。

    两个问题都成立，错的是**只有一个范围、却被当成两个意思用**。
    所以把它显式拆开：工作台点进来落在「我负责的」，数字对得上；
    想看有没有活没人认领，切到「与我相关」。
  */
  { value: 'mine' as const, label: '我负责的', title: '负责人是我、或我负责其中服务项、或有任务在我名下的项目。**不含**还没人认领的项目' },
  { value: 'related' as const, label: '与我相关', title: '在「我负责的」基础上，**加上**还没人认领的项目（它对所有人可见，否则谁都看不到它）' },
