...
/*
  ══════════════════════════════════════════════════════════════
  颜色语义 —— 什么时候用哪个色（2026-09-21 补）
  ══════════════════════════════════════════════════════════════

  金恩来：「系统中有很多颜色，不同的颜色代表不同的意思，问题是用在
    不同的页面、功能、板块、数据上面的时候有统一过标准规范没有？」

  **没有。** 色板（下面那 8 个 tone）早就有了，但从来没写过
  "什么时候用哪个" —— 于是同一件事在不同页面是不同颜色，
  而同一个颜色在不同页面是不同意思。

  实测（2026-09-21 扫 pages/ + components/）还更糟：
  **13 个色系在用，而色板只定义了 8 个**，多出来的全是同义重复：
  ...
  同一个意思两套颜色，没有任何好处 —— 只会让人以为"这两个不一样"。

  ── 语义约定（只此一份，改这里，不要在页面上自己发挥）──────────

    red      **出事了，会造成损失** —— 逾期、超额、数据冲突、删除类动作
    amber    **要注意，但还没出事** —— 临期、待补信息、未核实、弱匹配
    emerald  **成了 / 通过 / 已完成** —— 已到账、已验证、进度达标
    blue     **主行动** —— 新建、提交、确认这类"往前走一步"的按钮
    indigo   **AI 和智能功能** —— 识别、提炼、AI 摘要、智能助手
    purple   **参考资料类** —— 话术、模板、知识文档
    orange   **要人做决定的提醒** —— 待审批、待确认、排提醒
    gray     **中性/未激活** —— 空状态、禁用、次要信息

  两条铁律：
    1. **红色只留给"已经出事"**。临期、待补这类用 amber ——
       什么都标红，红色就不再意味着紧急。
    2. **一个意思只能一个色**。要新的意思就先在这里加一个 tone，
       不要在页面里挑一个没人用过的色系。

  配套闸门：tests/color-semantics.test.js 盯着同义色系不许再冒出来。
*/
export type Tone = 'gray' | 'blue' | 'indigo' | 'emerald' | 'amber' | 'red' | 'orange' | 'purple';

/** 每个 tone 是什么意思 —— 给人看的，也给测试对账用 */
export const TONE_MEANING: Record<Tone, string> = {
  red: '出事了，会造成损失（逾期、超额、冲突、删除）',
  amber: '要注意，但还没出事（临期、待补、未核实）',
  emerald: '成了 / 通过 / 已完成',
  blue: '主行动（新建、提交、确认）',
  indigo: 'AI 和智能功能',
  purple: '参考资料（话术、模板、知识）',
  orange: '要人做决定的提醒（待审批、待确认）',
  gray: '中性 / 未激活 / 空状态'
};

/**
 * 被禁用的同义色系 —— 每个都已经有对应的 tone 了。
 * 值是"该用哪个代替"，测试直接拿它出错误提示。
 */
export const BANNED_COLOR_FAMILIES: Record<string, Tone> = {
  green: 'emerald',
  slate: 'gray',
  yellow: 'amber',
  rose: 'red',
