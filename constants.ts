...
export const ROLE_CAPABILITIES: Record<RoleID, RoleCapability> = {
  ...
  CONSULTANT: {
    actions: [
      /*
        ── 顾问可以自己立项（2026-09-07 放开）──────────────────

        原来只有总经理和总助能立项，理由是「项目由上面派、顾问执行」。
        这话在流程图上成立，在 13 个人的公司里不成立：

        顾问在客户现场发现要多做一项体系、或者要开一个跟进项目，
        按原来的规矩他得回来找总助，总助再找他确认细节。
        **一件他自己最清楚的事，要绕两个人。**
        绕不动的时候，他就记在自己本子上 —— 系统里反而没有了。

        风险是可能多出一些不该有的项目。但那是**看得见、能删的**，
        而漏掉的项目是看不见的。两害相权，取看得见的那个。

        每个项目都记着是谁建的（业务账本），乱建有人负责。
      */
      'PROJECT_VIEW', 'PROJECT_CREATE', 'PROJECT_ASSIGN_MANAGER',
...
export const ROLE_TO_PERSONA: Record<RoleID, DashboardPersona> = {
  ...
  MANAGER: 'manager',      // 总助有自己的看板（派活/延期/日志），见 ManagerDashboard
  SALES: 'sales',
